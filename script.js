...
window.addEventListener('scroll', () => {
  ...
  nav.style.background = window.scrollY > 10 ? 'rgba(8,16,43,0.98)' : 'rgba(8,16,43,0.9)';
});

/* ═══════════════════════════════════════════
   VIDEO MODAL (YouTube)
   Dán video ID vào data-yt-id của #videoModal
═══════════════════════════════════════════ */
...
function openVideo(e) {
  ...
  if (!ytId) { console.warn('Chưa gắn YouTube ID!'); return; }
...
videoModal.addEventListener('click', e => { if (!videoContainer.contains(e.target)) closeVideo(); });
...
const members = [
  ...
  { name: "Nguyễn Nam Thành",       role: "Embedded Systems & SW Engineering",img: "assets/Thành.jpg",   init: "Nguyễn Nam Thành",   fb: "https://www.facebook.com/thanh.nguyennam.31521?locale=vi_VN",        gh: "https://github.com/namthanh82" },
...
const featuredAdvisor = {
  ...
  init: "PGS.TS", img: "assets/Thầy.png",
  desc: [
...
function getVisible() {
  ...
  if (w <= 500) return 1; if (w <= 768) return 2; if (w <= 1100) return 3; return 4;
}
function maxIndex() { return achCards.length - visibleCount; }
function buildDots() {
...
buildDots(); goTo(0);
window.addEventListener('resize', () => { visibleCount = getVisible(); buildDots(); goTo(Math.min(current, maxIndex())); });
achCards.forEach(card => {
  const link = card.querySelector('.read-more');
  card.addEventListener('click', e => { if (!e.target.closest('.read-more')) link.click(); });
});
...
function addRevealToCards(containerId) {
  ...
  const delays = ['delay-1','delay-2','delay-3','delay-4','delay-5','delay-6','delay-1','delay-2','delay-3'];
